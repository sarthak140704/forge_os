...
//!
//! A **bundle** is a JSON file with three fields:
//! ```json
//! { "manifest": <yaml frontmatter dict>,
...
//! ```
//!
//! The signed bytes are `serde_json::to_vec(&{manifest, files})` — the
//! signature covers manifest + all file contents, no more, no less. Two
//! bundles built from the same directory produce byte-identical signed bytes,
...
//! * **No compression** — skill files are already small markdown; zipping
//!   just adds surface area.
...
//!   whether that key is trusted. This CLI writes the pubkey next to the
//!   private key when `keygen` runs, so a common workflow is: alice publishes
//!   `alice.pub`, users add it to `~/.config/forge/trusted-keys/`, and the
//!   `install` subcommand refuses bundles whose pubkey isn't there.
...
pub struct Bundle {
    /// Frontmatter of the skill file — arbitrary YAML dict.
    pub manifest: serde_yaml::Value,
    /// Every file under the skill dir, keyed by forward-slash relative path.
...
pub fn sign_bundle(skill_dir: &Path, out: &Path, priv_key_path: &Path) -> Result<()> {
    if !skill_dir.is_dir() {
    ...
    let (manifest, files) = collect_skill_files(skill_dir)?;

    // Sign the canonical bytes.
    let payload = Signed { manifest: &manifest, files: &files };
    ...
    println!("wrote signed bundle: {}  ({} files, pubkey {})",
        out.display(),
        bundle.files.len(),
...
pub fn verify_bundle(bundle_path: &Path, expected_pubkey_path: Option<&Path>) -> Result<Bundle> {
    ...
    let payload = Signed { manifest: &bundle.manifest, files: &bundle.files };
