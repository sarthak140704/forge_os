...
Write-Host "     Phase 6c: `$env:ANTHROPIC_API_KEY / `$env:GEMINI_API_KEY are also picked up automatically."
Write-Host "     Phase 6g: Azure -> `$env:AZURE_OPENAI_API_KEY + `$env:AZURE_OPENAI_ENDPOINT + `$env:AZURE_OPENAI_DEPLOYMENT (optional `$env:AZURE_OPENAI_API_VERSION)."
Write-Host "     Phase 6g: local OpenAI-compatible servers -> `$env:LMSTUDIO_BASE_URL (e.g. http://127.0.0.1:1234/v1) or `$env:VLLM_BASE_URL (e.g. http://127.0.0.1:8000/v1)."
...
Write-Host "  4. Optional — build the headless CLI:"
Write-Host "       cargo install --path apps\forge-cli"
Write-Host "       forge --help"
Write-Host "  5. Optional — VS Code extension:"
Write-Host "       cd apps\forge-vscode; npm install; npm run compile; then press F5"
Write-Host "  6. Optional — messaging gateway (Slack + generic + Discord + Telegram):"
Write-Host "       cargo install --path apps\forge-gateway"
Write-Host "       `$env:FORGE_TOKEN = '<same as FORGE_API_TOKEN>'"
Write-Host "       `$env:DISCORD_APPLICATION_PUBLIC_KEY = '<hex>'     # Phase 6e — Discord opt-in"
Write-Host "       `$env:TELEGRAM_BOT_TOKEN            = '<bot-tok>' # Phase 6e — Telegram opt-in"
Write-Host "       forge-gateway"
Write-Host "  7. Optional — OpenTelemetry (Phase 6b):"
Write-Host "       `$env:FORGE_OTLP_ENDPOINT = 'http://localhost:4318'  # OTLP HTTP-protobuf"
Write-Host "       `$env:FORGE_OTEL_SERVICE_NAME = 'my-forge'          # optional override"
